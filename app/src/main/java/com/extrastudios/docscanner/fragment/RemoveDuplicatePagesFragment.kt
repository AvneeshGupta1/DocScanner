...
import kotlinx.android.synthetic.main.fragment_remove_duplicate_pages.*
import kotlinx.android.synthetic.main.storage_permission_view.*
import java.io.File
import java.util.*

class RemoveDuplicatePagesFragment : BaseFragments(), OnPDFCreatedInterface, OnBackPressedInterface,
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_remove_duplicate_pages, container, false)
    }
    ...
    private fun resetValues() {
        mPath = null
        mMorphButtonUtility!!.initializeButton(selectFile, remove!!)
    }

    private fun setTextAndActivateButtons(path: String) {
        mPath = path
        mMorphButtonUtility!!.setTextAndActivateButtons(path, selectFile, remove!!)
    }
    ...
    override fun onPDFCreated(success: Boolean, path: String?) {
        ...
        if (!success) {
            showSnackbar(R.string.snackbar_no_duplicate_pdf)
            view_pdf!!.hide()
            return
        }
    ...
    override fun onClick(view: View?) {
        when (view) {
            ...
            remove -> RemoveDuplicates(mPath, this).execute()
